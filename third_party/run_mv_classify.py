...
def train(args, train_dataset, dropped_train_dataset, model, tokenizer, lang2id=None):
    ...
    train_sampler = RandomSampler(concat_train_dataset) if args.local_rank == -1 else DistributedSampler(
    ...
    print(
        f'SB-DEBUG Inside train function saved optimizer path:{os.path.join(args.model_name_or_path, "optimizer.pt")}')
    print(
        f'SB-DEBUG Inside train function saved scheduler path:{os.path.join(args.model_name_or_path, "scheduler.pt")}')
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.save_only_best_checkpoint:
                        ...
                        logger.info(
                            " Dev accuracy on train_language {} = {}".format(args.train_language, result['acc']))
...
def main():
    ...
    if args.do_train:
        ...
        dropped_train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, split=args.train_split,
                                                        language=args.train_language, lang2id=lang2id,
                                                        bpe_drop=args.bpe_dropout, evaluate=False)
        global_step, tr_loss, best_score, best_checkpoint = train(args, train_dataset, dropped_train_dataset, model,
                                                                  tokenizer, lang2id)
